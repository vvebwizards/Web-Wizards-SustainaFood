...
const RoomHeader: React.FC<RoomHeaderProps> = ({
  ...
  const canInvite = selectedRoom.createdBy === currentUserId || 
                    selectedRoom.admins?.includes(currentUserId);
  ...
  return (
    <>
      <div className="px-6 py-3 border-b border-gray-200 flex items-center justify-between bg-white sticky top-0 z-10">
        ...
        {selectedRoom.isPrivate && canInvite && (
          <button
            onClick={() => setShowInviteModal(true)}
            className={`ml-2 px-3 py-1 ${theme.button} text-white rounded text-sm hover:opacity-90 transition-opacity`}
          >
            Invite
          </button>
        )}
