...
const AllDonations = () => {
  ...
  useEffect(() => {
    const fetchDonations = async () => {
      try {
        const response = await axios.get("https://foodreduce-backend.azurewebsites.net/api/foodItem/getAllDonationFood");
        const donationsWithUsernames = await Promise.all(
          response.data.map(async (donation: Donation) => {
            if (donation.donorId) {
              try {
                const userResponse = await axios.get(`https://foodreduce-backend.azurewebsites.net/api/users/getUser/${donation.donorId}`);
